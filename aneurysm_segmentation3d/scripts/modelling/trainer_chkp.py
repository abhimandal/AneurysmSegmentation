...
class Trainer:
    ...
    def _initialize_trainer(self):
        ...
        self._checkpoint.dataset_properties = (
            self._dataset.used_properties
        )
    ...
    def fit(self):
        ...
        for epoch in range(
            self._checkpoint.start_epoch,
            self._cfg.training.epochs + 1,
        ):
            log.info(
    ...
    def _test_epoch(self, epoch, stage_name: str):
        ...
        for loader in loaders:
            ...
            if not self._dataset.has_labels(
                ...
                log.warning(
                    "No forward will be run on dataset %s."
                    % stage_name
                )
                continue
    ...
    def _finalize_epoch(self, epoch):
        ...
        if self._is_training:
            ...
            if self._tracker._stage == "train":
                log.info(
                    "Learning rate = %f" % self._model.learning_rate
                )
